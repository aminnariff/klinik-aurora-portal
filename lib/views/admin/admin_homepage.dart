...
class _AdminHomepageState extends State<AdminHomepage> {
  ...
  Widget searchField({required TextEditingController controller, required String hintText}) {
    return Padding(
      padding: EdgeInsets.all(screenPadding),
      child: SizedBox(
        height: 40,
      ...
      child: Row(
        children: [
          ...
          ElevatedButton.icon(
            ...
            label: const Text('Add Admin', style: TextStyle(color: Colors.white, fontSize: 13)),
                  ...
                  builder: (ctx, setFilterState) {
                    return Column(
                      ...
                      children: [
                        ...
                        _filterLabel('Admin Name'),
                        const SizedBox(height: 6),
                        _filterTextField(_userNameController, 'Search by name'),
                        const SizedBox(height: 16),
                        _filterLabel('Phone Number'),
                        const SizedBox(height: 6),
                        _filterTextField(_userPhoneController, 'Search by phone'),
                        const SizedBox(height: 16),
                        _filterLabel('Email'),
                        const SizedBox(height: 6),
                        _filterTextField(_userEmailController, 'Search by email'),
                        const SizedBox(height: 16),
                        _filterLabel('Status'),
                        const SizedBox(height: 6),
            ...
            Expanded(
              child: DataTable2(
                ...
                columns: [
                  ...
                  _col('Email', ColumnSize.L),
                  _col('Contact', ColumnSize.S, fixedWidth: 140),
                  ...
                  for (int i = 0; i < items.length; i++)
                    DataRow2(
                      ...
                      cells: [
                          ...
                          GestureDetector(
                            onTap: () {
                              showDialog(
                                context: context,
                                builder: (BuildContext context) {
                                  return AdminDetail(user: items[i], type: 'update');
                                },
                              );
                            },
                            child: Text(
                              items[i].userFullname ?? items[i].userName ?? 'N/A',
                        ...
                        DataCell(
                          Text(
                            items[i].userEmail ?? 'N/A',
                            style: const TextStyle(fontSize: 12, color: Color(0xFF6B7280)),
                          ),
                        ),
                        DataCell(
                          Text(
                            items[i].userPhone ?? 'N/A',
                            style: const TextStyle(fontSize: 12, color: Color(0xFF374151)),
                          ),
                        ),
                        DataCell(_statusChip(items[i].userStatus == 1)),
                        DataCell(
                          Text(
      ...
      itemBuilder: (_) => [
        const PopupMenuItem<String>(
          ...
          child: Row(children: [
            ...
            SizedBox(width: 10),
            Text('Edit Admin', style: TextStyle(fontSize: 13)),
          ]),
        ),
        PopupMenuItem<String>(
          ...
          child: Row(children: [
            Icon(
              data.userStatus == 1 ? Icons.block_rounded : Icons.check_circle_outline_rounded,
              size: 16,
              color: data.userStatus == 1 ? Colors.red : Colors.green,
            ),
            const SizedBox(width: 10),
            Text(
              data.userStatus == 1 ? 'Deactivate' : 'Activate',
              style: TextStyle(fontSize: 13, color: data.userStatus == 1 ? Colors.red : Colors.green),
            ),
          ]),
        ),
      ],
  ...
  Future<void> _toggleAdminStatus(Data data) async {
    try {
